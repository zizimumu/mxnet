...
# 第2层网络及非线性激活 64
#fc4  = mx.sym.FullyConnected(data=act2, num_hidden=64, name="fc4")
#act4 = mx.sym.Activation(data=fc4, act_type="relu", name="act4")
...
module.fit(
    ...
    eval_data=val_iter,
	#eval_data=None,
	eval_metric = mx.metric.create('ce'), # 输出 MSE 损失信息
    optimizer = 'sgd',
# 采用0.2的初始学习速率，并在每60000个样本后（即每1个epoch后）将学习速率缩减为之前的0.9倍
    #optimizer_params = {'learning_rate' : 0.1, 'lr_scheduler' : mx.lr_scheduler.FactorScheduler(step=60000/batch_size, factor=0.9)},
    optimizer_params = {'learning_rate' : 0.03},
    num_epoch = 100,
    #batch_end_callback = mx.callback.Speedometer(batch_size, 60000/batch_size),
    #batch_end_callback = mx.callback.log_train_metric(60000/batch_size,True)
)
